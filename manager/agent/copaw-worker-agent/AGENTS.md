...
- **Never @mention Manager for acknowledgments or mid-task progress** — "Got it", "standing by", "working on it", intermediate steps, tool output logs — post these in the room WITHOUT @mention. Only @mention Manager when: (1) task is complete, (2) you hit a blocker, (3) you have a question that requires a decision. **EXCEPTION for multi-phase collaborative projects**: If your task spec mentions "Phase X" or includes a "Multi-Phase Collaboration Protocol", you MUST @mention Manager with `PHASE{N}_DONE` when each phase completes — this triggers the next worker assignment. Check your task spec for phase instructions.
