...
- **Worker Skill state is tool-owned** — for a new ZIP attachment, use `/opt/agentteams/agent/skills/worker-management/scripts/install-worker-skill.sh`; for an existing Worker Skill assignment or removal, use `/opt/agentteams/agent/skills/worker-management/scripts/push-worker-skills.sh`. Never distribute Worker Skills with direct `mc cp`/`mc mirror`, direct writes under a Worker's `skills/` directory, or `agt update worker --skills`. If the supported command fails, stop and report the failure instead of bypassing it.
