...
- **权限治理体系**：可以复用 MCP 的认证鉴权能力（工具粒度的权限管理）
...
- **MCP 负责细粒度权限管控**：贴合业务能力做好工具级别的权限治理和凭证管理
...
- **Slash Commands** — `/reset` 清除上下文，`/stop` 中断长时间任务。在 DM 和群聊中都可用。
- **优化的文件同步** — "写入者推送并通知，接收者按需拉取"设计原则。
