...
- **User-Defined Env Vars on Worker/Manager/Team CRs** — Worker, Manager, `Team.leader`, and `Team.workers[]` CRs now honor a new `spec.env` (`map[string]string`) field. User-declared variables are injected into the target container alongside the controller/backend-managed ones, with system-produced keys winning any collision (ignored user keys are logged once at INFO with the dropped key list). Merge is wired at `createMemberContainer` (covers standalone Workers and both Team roles via the existing `WorkerSpec` projection) and `createManagerContainer`; the `WorkerEnvBuilder` stays pure. CRDs add an `env` property block with `additionalProperties.type=string` and `propertyNames.pattern=^[a-zA-Z_][a-zA-Z0-9_]*$` so non-POSIX keys are rejected at admission. `hiclaw` CLI / REST API surface is intentionally deferred — apply env via `kubectl apply -f` directly for now.
...
- **Worker/Manager/Team CR 支持用户自定义环境变量** — Worker、Manager、`Team.leader` 和 `Team.workers[]` CRD 新增 `spec.env`（`map[string]string`）字段。用户声明的变量会与 Controller/Backend 产出的系统变量一起注入目标容器，冲突时系统侧始终获胜（被忽略的用户 key 会在 INFO 日志里排序输出一次）。合并点在 `createMemberContainer`（覆盖独立 Worker 及 Team Leader/Worker 两种角色的 `WorkerSpec` 投射）和 `createManagerContainer`；`WorkerEnvBuilder` 本身保持纯函数。CRD 增加 `env` 属性块，`additionalProperties.type=string` + `propertyNames.pattern=^[a-zA-Z_][a-zA-Z0-9_]*$`，非 POSIX key 会被 admission 拒绝。`hiclaw` CLI / REST API 暂不开放此字段，直接用 `kubectl apply -f` 即可。
